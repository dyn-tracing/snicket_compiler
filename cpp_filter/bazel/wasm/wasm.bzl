...
def _wasm_transition_impl(settings, attr):
    return {
        ...
        "//command_line_option:cxxopt": ["-std=c++17", "-O3", "-no-canonical-prefixes", "-DEMSCRIPTEN_PROTOBUF_LITE=1", "-fdata-sections", "-ffunction-sections"],
        "//command_line_option:linkopt": ["-Wl", "--gc-sections"],
...
def wasm_cc_binary(name, **kwargs):
    ...
    kwargs.setdefault("additional_linker_inputs", ["@envoy_wasm_api//:jslib"])
    kwargs.setdefault("linkopts", ["--js-library external/envoy_wasm_api/proxy_wasm_intrinsics.js"])
