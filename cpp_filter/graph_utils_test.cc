...
TEST(GraphUtilsTest, IsoSanityTestDirectedGraph) {
    typedef boost::directed_graph<> graph_type;
...
TEST(GraphUtilsTest, DirectedGraphWithProperties) {
    ...
    Node node2;
    node2.properties.insert({{"workload_name"}, "productpage-v1"});
    auto v0 = graph1.add_vertex();
    auto v1 = graph1.add_vertex();
    ...
    graph_type graph3;
    Node node0;
    node0.properties.insert({{"workload_name"}, "productpage-v1"});
...
TEST(GraphUtilsTest, DirectedGraphPropertySubset) {
    ...
    graph_type graph2;
    Node node1;
    node1.properties.insert({{"workload_name"}, "productpage-v1"});
...
TEST(GetSubGraphMappintTest, NoMapping) {
    trace_graph_t graph_small =
        generate_trace_graph_from_headers("a", "a.x==y");
    trace_graph_t graph_large =
        generate_trace_graph_from_headers("b", "b.x==z");
