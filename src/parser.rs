...
fn parse_patterns<'a>(token_iter : &mut TokenIterator<'a>) -> Patterns<'a> {
  let is_pattern = |token| { match token { &Token::Match => true, _ => false, } };
...
fn parse_pattern<'a>(token_iter : &mut TokenIterator<'a>) -> Pattern<'a> {
  match_token(token_iter, Token::Match, "Pattern must start with the keyword MATCH.");
  ...
  return Pattern{from_node, to_node, relationship_type};
}
...
mod tests {
  ...
  test_parser_fail!   (r"n", parse_pattern, test_parse_pattern_fail,
  ...
  test_parser_fail!   (r"n", parse_filter, test_parse_filter_fail,
                       "Invalid token: Identifier(\"n\"), expected Where");
  test_parser_success!(r"MATCH n-->m MATCH n-*>m WHERE n:Node WHERE m:Node WHERE
                         n.abc == 5", parse_prog, test_parse_prog);
  test_parser_success!(r"MATCH n-->m MATCH n-*>m", parse_prog, test_parse_prog_no_filter);
  test_parser_fail!   (r"n-->m", parse_prog, test_parse_prog_fail,
